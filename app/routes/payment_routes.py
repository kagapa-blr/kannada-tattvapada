...
def create_order():
    try:
        if not request.is_json:
            return jsonify(MessageTemplate.invalid_request("Invalid JSON body")), 400
        ...
        if not email or not phone or amount is None:
            return jsonify(
                MessageTemplate.invalid_request("Email, phone, and amount are required")
            ), 400
        ...
        try:
            ...
            return jsonify(
                MessageTemplate.invalid_request("Amount must be a positive number")
            ), 400
...
# Payment Status
# =====================================================
@payment_bp.route(f"{API_PREFIX}/payment-status", methods=["GET"])
def payment_status():
    ...
    try:
        ...
        status = (
            ...
            or getattr(order, "status", "PENDING")
        )

        receipt_data = {
            "Order ID": getattr(order, "order_id", order_id),
            "Status": status,
            "Amount": payment.get("payment_amount")
                      or getattr(order, "order_amount", 0),
            "Transaction No": payment.get("cf_payment_id") or "N/A",
            "Transaction Date": payment.get("payment_time")
                               or getattr(order, "created_at", None),
            "Bank Ref No": payment.get("bank_reference") or "N/A",
            "Payment Method": payment.get("payment_method")
                              or getattr(order, "payment_method", None),
            "Customer Info": getattr(order, "customer_details", {}),
...
def webhook():
    try:
        ...
        if not signature or not timestamp:
            return jsonify(
                MessageTemplate.invalid_request("Missing webhook headers")
            ), 400

        event_type, event_data = payment_service.verify_webhook(
            signature,
            raw_body,
            timestamp
        )
        ...
        logger.info(
            f"[Webhook] Received {event_type} for order {event_data.get('order_id')}"
        )
...
def success_page():
    ...
    try:
        ...
        if status and status.upper() in ["SUCCESS", "PAID"]:
