...
class CashfreePaymentService:
    ...
    def create_order(self, email: str, phone: str, amount: float, name: str = None,
        ...
        """Create a Cashfree order and save it in the local DB."""
        ...
        create_order_request = CreateOrderRequest(
            ...
            order_note=kwargs.get("order_note", "Flask Cashfree Hosted Checkout")
        )
        ...
        if not payment_session_id:
            self.logger.error("Payment Session ID not found in Cashfree response")
    ...
    def fetch_order_with_payments(self, order_id: str):
        """Fetch order details + associated payments from Cashfree API."""
        ...
        # 2. Fetch payment details via REST API
        ...
        if payments_response.status_code != 200:
            self.logger.warning(
                f"Failed to fetch payments for {order_id}: {payments_response.text}"
            )
            payment = {}
        else:
        ...
        # ✅ Normalize payment details (safe access)
        payment_info = {
    ...
    def verify_webhook(self, signature: str, raw_body: str, timestamp: str):
        """Verify webhook and update local order status."""
